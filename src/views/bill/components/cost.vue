  ...
  <div class="parent-bill-cost">
    ...
    <Table :header="state.headerTable">
      <template v-slot:Larg>
        <tr v-for="(food, index) in props.bill.bill_foods" :key="food.id">
          <td>{{ index + 1 }}</td>
          <td>{{ food.name }}</td>
      ...
      <template v-slot:small>
        <div v-for="(food, index) in props.bill.bill_foods" class="small-table" :key="food.id">
    ...
    <!-- ///////////////////////// -->
    <div class="flex flex-col w-full justify-start mt-2 items-center rounded-md">
      <!-- ///////////////////////// -->
      ...
      <div class="cost-total">
        <p>هزنیه بازی شده</p>
        <p>
          {{ props.bill.total_price.toLocaleString() }}
          تومان
        </p>
      </div>
      <!-- ///////////////////////// -->
      <div class="cost-total bg-white text-black font-bold rounded-b-md">
        <p>هزینه کل</p>
        <p>
          {{ (props.bill.total_price + props.bill.foodCost).toLocaleString() }}
          تومان
        </p>
      </div>
      <!-- ///////////////////////// -->
