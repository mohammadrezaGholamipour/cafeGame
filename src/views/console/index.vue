...
const consoleData = computed(() => {
  if (
    ...
    return pinia.state.console.map(({ id, name, is_deleted }: consoleTypeApi) => {
      /////////////////////////////
      const item = {
        id: 0,
        name: "",
...
<template>
  <div class="parent-console-page">
    <!-- /////////////////////////////////////// -->
    <tools @displayMode="pinia.handleChangeDisplayMood($event)" :loading="state.requestLoading"
    ...
    <transition-fade group class="w-full overflow-y-auto h-full">
      <div v-if="consoleData?.length" class="parent-console">
        ...
        <component :is="pinia.state.displayMood === 1 ? ConsoleLine : ConsoleBox" @remove="handleRemoveConsole(console)"
          :loading="state.requestLoading" v-for="console in consoleData.sort((a, b) => a.name.localeCompare(b.name))"
          :name="console.name" :id="console.id" :playedCost="console.playedCost" :playedTime="console.playedTime"
          :key="console.id" :isDeleted="console.isDeleted" />
      </div>
      <img src="@/assets/image/noData.svg" v-else-if="consoleData" class="w-full h-full" />
      <loading v-else />
    </transition-fade>
    <!-- //////////////////////////////////////// -->
    <Dialog :status="state.removeConsole.dialogStatus" @changeStatus="handleStatusDialog" :btnCancelText="'بازگشت'"
